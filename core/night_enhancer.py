...
class NightEnhancer:
    ...
    def __init__(self, brightness_threshold: float = 65.0, clip_limit: float = 3.0, tile_grid_size: Tuple[int, int] = (8, 8)):
    ...
    def process(self, frame: np.ndarray, camera_id: str = "camera1", camera_name: str = "Camera 1", location: str = "Border Sector") -> Tuple[np.ndarray, bool, Optional[AlertEvent]]:
        ...
        # Convert to grayscale to measure average scene brightness
        ...
        if new_night_state != self.is_night_mode:
            ...
            state_change_alert = AlertEvent(
                ...
                metadata={"brightness": round(mean_brightness, 2), "mode": mode_str}
            )
