...
export default function ManageView({ onOpenSnapshot, cameras = [], onRefreshCameras, initialPersons = [], onRefreshPersons }) {
  ...
  // Modal state for Edit Camera
  const [showEditCamModal, setShowEditCamModal] = useState(false);
  const [editCamData, setEditCamData] = useState(null);
  const [editCamLoading, setEditCamLoading] = useState(false);
  const [editCamError, setEditCamError] = useState('');
  ...
  const handleOpenEditCamModal = (cam) => {
    const cid = cam.camera_id || cam.id;
    setEditCamData({
      id: cid,
      name: cam.camera_name || cam.name || cid,
      location: cam.location || '',
      ip_address: cam.ip_address || '',
      rtsp_url: cam.rtsp_url || '',
      conf_threshold: cam.conf_threshold ?? 0.25,
      enable_face_detection: cam.enable_face_detection !== false,
      enable_anpr: cam.enable_anpr !== false,
      enable_night_mode: cam.enable_night_mode !== false,
      virtual_fence: cam.virtual_fence ? { ...cam.virtual_fence } : {
        enabled: false,
        type: 'horizontal',
        position: 50,
    ...
    setEditCamError('');
    setShowEditCamModal(true);
  };

  // Save Camera Edit Changes
  const handleSaveEditCamera = async (e) => {
    e.preventDefault();
    if (!editCamData) return;
    setEditCamLoading(true);
    setEditCamError('');
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 15000);
      const res = await fetch(`/api/cameras/${editCamData.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        signal: controller.signal,
        body: JSON.stringify({
          name: editCamData.name,
          location: editCamData.location,
          ip_address: editCamData.ip_address,
          rtsp_url: editCamData.rtsp_url,
          conf_threshold: parseFloat(editCamData.conf_threshold),
          enable_face_detection: editCamData.enable_face_detection,
          enable_anpr: editCamData.enable_anpr,
          enable_night_mode: editCamData.enable_night_mode,
          virtual_fence: editCamData.virtual_fence
        })
      });
      clearTimeout(timeoutId);
      if (res.ok) {
        setShowEditCamModal(false);
        if (onRefreshCameras) onRefreshCameras();
      } else {
        const errData = await res.json().catch(() => ({}));
        setEditCamError(errData.detail || 'Failed to update camera.');
      }
    } catch (err) {
      if (err.name === 'AbortError') {
        setEditCamError('Request timed out. Please try again.');
      } else {
        setEditCamError('Network error: ' + err.message);
      }
    } finally {
      setEditCamLoading(false);
    }
  };
  ...
  const handleDeleteCamera = async (camId, camName) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errData = await res.json().catch(() => ({}));
        alert(`Failed to delete camera: ${errData.detail || res.statusText}`);
      }
    } catch (err) {
  ...
  return (
    <div className="manage-view-container">
            ...
            {cameras.map((cam) => {
              ...
              return (
                <div key={cid} style={{
                  ...
                  {/* Virtual Fence Status Summary */}
                  <div style={{
                    background: cam.virtual_fence?.enabled ? '#fff5f5' : '#f8fafc',
                    border: cam.virtual_fence?.enabled ? '1px solid #fecaca' : '1px solid #e2e8f0',
                    borderRadius: '8px',
                    padding: '8px 12px',
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Shield size={14} color={cam.virtual_fence?.enabled ? '#ef4444' : '#64748b'} />
                      <span style={{ fontSize: '0.78rem', fontWeight: 600, color: cam.virtual_fence?.enabled ? '#b91c1c' : '#475569' }}>
                        Virtual Fence: {cam.virtual_fence?.enabled ? `Active (${cam.virtual_fence?.type === 'vertical' ? 'Vertical' : 'Horizontal'} ${Math.round(cam.virtual_fence?.position ?? 50)}%)` : 'Disabled'}
                      </span>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleOpenEditCamModal(cam)}
                      style={{
                        padding: '2px 8px',
                        fontSize: '0.72rem',
                        fontWeight: 600,
      ...
      {/* EDIT CAMERA MODAL */}
      {showEditCamModal && editCamData && (
        <div
          className="modal-backdrop"
          onClick={() => setShowEditCamModal(false)}
          style={{
            position: 'fixed',
            top: 0,
            left: 0,
        ...
        >
          ...
          >
            ...
            {/* Form */}
            <form onSubmit={handleSaveEditCamera} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {editCamError && (
                <div style={{
                  background: '#fef2f2',
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditCamData({ ...editCamData, name: e.target.value })}
                  style={{
                    width: '100%',
                    padding: '8px 12px',
                    fontSize: '0.88rem',
              ...
              <div>
                ...
                <input
                  type="text"
                  value={editCamData.location}
                  onChange={(e) => setEditCamData({ ...editCamData, location: e.target.value })}
                  style={{
                    width: '100%',
                    padding: '8px 12px',
                    fontSize: '0.88rem',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input
                    type="text"
                    value={editCamData.ip_address}
                    onChange={(e) => setEditCamData({ ...editCamData, ip_address: e.target.value })}
                    style={{
                      width: '100%',
                      padding: '8px 12px',
                      fontSize: '0.88rem',
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.84rem', fontWeight: 600, color: '#1e293b', marginBottom: '4px' }}>
                    Confidence Threshold ({Math.round(editCamData.conf_threshold * 100)}%)
                  </label>
                  <input
                    type="range"
                    min="0.10"
                    max="0.90"
                    step="0.05"
                    value={editCamData.conf_threshold}
                    onChange={(e) => setEditCamData({ ...editCamData, conf_threshold: parseFloat(e.target.value) })}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={editCamData.rtsp_url}
                  onChange={(e) => setEditCamData({ ...editCamData, rtsp_url: e.target.value })}
                  style={{
                    width: '100%',
                    padding: '8px 12px',
                    fontSize: '0.88rem',
              ...
              <div style={{
                ...
                <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#334155' }}>Active AI Modules</span>
                <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.82rem', cursor: 'pointer' }}>
                    <input
                      type="checkbox"
                      checked={editCamData.enable_face_detection}
                      onChange={(e) => setEditCamData({ ...editCamData, enable_face_detection: e.target.checked })}
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.82rem', cursor: 'pointer' }}>
                    <input
                      type="checkbox"
                      checked={editCamData.enable_anpr}
                      onChange={(e) => setEditCamData({ ...editCamData, enable_anpr: e.target.checked })}
                      style={{ accentColor: '#2563eb' }}
                    />
                    ANPR Plates
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.82rem', cursor: 'pointer' }}>
                    <input
                      type="checkbox"
                      checked={editCamData.enable_night_mode}
                      onChange={(e) => setEditCamData({ ...editCamData, enable_night_mode: e.target.checked })}
                      style={{ accentColor: '#2563eb' }}
                    />
                    CLAHE Night
                  </label>
              ...
              {/* Virtual Fence in Modal */}
              <div style={{
                background: editCamData.virtual_fence?.enabled ? '#fff5f5' : '#f8fafc',
                border: '1px solid #e2e8f0',
                borderRadius: '10px',
                padding: '12px 14px',
                display: 'flex',
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '0.82rem', fontWeight: 700, color: editCamData.virtual_fence?.enabled ? '#b91c1c' : '#334155' }}>
                    Virtual Fence Boundary
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                      const cur = editCamData.virtual_fence || { type: 'horizontal', position: 50, name: 'Virtual Fence' };
                      setEditCamData({
                        ...editCamData,
                        virtual_fence: { ...cur, enabled: !cur.enabled }
                      });
                    }}
                    style={{
                      padding: '3px 8px',
                      ...
                      fontWeight: 700,
                      borderRadius: '14px',
                      border: editCamData.virtual_fence?.enabled ? '1px solid #ef4444' : '1px solid #cbd5e1',
                      background: editCamData.virtual_fence?.enabled ? '#ef4444' : '#ffffff',
                      color: editCamData.virtual_fence?.enabled ? '#ffffff' : '#64748b',
                      cursor: 'pointer'
                    }}
                  >
                    {editCamData.virtual_fence?.enabled ? 'ON' : 'OFF'}
                  </button>
                </div>

                {editCamData.virtual_fence?.enabled && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button
                        type="button"
                        onClick={() => setEditCamData({
                          ...editCamData,
                          virtual_fence: { ...(editCamData.virtual_fence || {}), type: 'horizontal', enabled: true }
                        })}
                        style={{
                          flex: 1,
                          padding: '4px 8px',
                          fontSize: '0.74rem',
                          fontWeight: editCamData.virtual_fence?.type !== 'vertical' ? 700 : 500,
                          background: editCamData.virtual_fence?.type !== 'vertical' ? '#fee2e2' : '#ffffff',
                          border: editCamData.virtual_fence?.type !== 'vertical' ? '1px solid #ef4444' : '1px solid #e2e8f0',
                          color: editCamData.virtual_fence?.type !== 'vertical' ? '#991b1b' : '#475569',
                          borderRadius: '4px',
                          cursor: 'pointer'
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={() => setEditCamData({
                          ...editCamData,
                          virtual_fence: { ...(editCamData.virtual_fence || {}), type: 'vertical', enabled: true }
                        })}
                        style={{
                          flex: 1,
                          padding: '4px 8px',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span style={{ fontSize: '0.74rem', color: '#64748b' }}>Position:</span>
                      <input
                        type="range"
                        min="5"
                        max="95"
                        step="1"
                        value={editCamData.virtual_fence?.position ?? 50}
                        onChange={(e) => setEditCamData({
                          ...editCamData,
                          virtual_fence: { ...(editCamData.virtual_fence || {}), position: parseFloat(e.target.value), enabled: true }
                        })}
                        style={{ flex: 1, accentColor: '#ef4444', height: '4px', cursor: 'pointer' }}
                      />
                      <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#b91c1c' }}>
                        {Math.round(editCamData.virtual_fence?.position ?? 50)}%
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '6px' }}>
                ...
                >
                  {editCamLoading ? 'Saving...' : 'Save Camera Changes'}
                </button>
              </div>
            </form>
          </div>
